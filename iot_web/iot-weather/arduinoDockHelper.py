...
class ArduinoDock:
    def __init__(self, uart="/dev/ttyS1", baudRate=9600):
        try:
            ...
            # 1 serialPort=serial.Serial(port, debit) #initialise port (open a serial port)
            print("opening serial port")
            self.port = serial.Serial(port=uart, baudrate=baudRate)
        except x:
            warnings.warn(
                "Can't open the serial port to Arduino Dock. Please close any applications using the serial port and try again.")
            raise x
        print("init complete")
